...
class TimeSeriesFeatureEngineer:
    ...
    def extract_rolling_features(self, 
        ...
        features[f'{value_col}_rolling_mean'] = df_sorted[value_col].rolling(
            window=window_size, min_periods=1
        ).mean()
        
        features[f'{value_col}_rolling_std'] = df_sorted[value_col].rolling(
            window=window_size, min_periods=1
        ).std().fillna(0)
        
        features[f'{value_col}_rolling_min'] = df_sorted[value_col].rolling(
            window=window_size, min_periods=1
        ).min()
        
        features[f'{value_col}_rolling_max'] = df_sorted[value_col].rolling(
            window=window_size, min_periods=1
        ).max()
        
        features[f'{value_col}_rolling_median'] = df_sorted[value_col].rolling(
            window=window_size, min_periods=1
        ).median()
        
        # Rolling percentiles
        features[f'{value_col}_rolling_q25'] = df_sorted[value_col].rolling(
            window=window_size, min_periods=1
        ).quantile(0.25)
        
        features[f'{value_col}_rolling_q75'] = df_sorted[value_col].rolling(
            window=window_size, min_periods=1
        ).quantile(0.75)
        
        # Coefficient of variation
        features[f'{value_col}_rolling_cv'] = (
    ...
    def extract_trend_features(self, 
                               df: pd.DataFrame,
                               value_col: str,
                               time_col: str = 'time') -> pd.DataFrame:
        """
    ...
    def extract_change_features(self, 
                                df: pd.DataFrame,
                                value_col: str,
                                time_col: str = 'time') -> pd.DataFrame:
        """
        ...
        # Percentage change
        features[f'{value_col}_pct_change_1h'] = df_sorted[value_col].pct_change(1).fillna(0)
        features[f'{value_col}_pct_change_3h'] = df_sorted[value_col].pct_change(3).fillna(0)
    ...
    def extract_statistical_features(self, 
        ...
        if len(recent_values) > 2:
            features[f'{value_col}_recent_skew'] = [stats.skew(recent_values)] * len(df_sorted)
            features[f'{value_col}_recent_kurtosis'] = [stats.kurtosis(recent_values)] * len(df_sorted)
        else:
        ...
        for patient_id, patient_df in df.groupby(patient_id_col):
            ...
            for col in numeric_cols:
                if col in patient_df.columns:
                    ...
                    patient_features = pd.concat([patient_features, rolling_feat], axis=1)
                    
                    # Trend features
                    trend_feat = self.extract_trend_features(
                        patient_df, col, time_col
                    )
                    ...
                    patient_features = pd.concat([patient_features, change_feat], axis=1)
